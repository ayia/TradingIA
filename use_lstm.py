...
from tensorflow.keras.layers import LSTM, Dropout, Dense, BatchNormalization, Attention
...
def load_json_file(file_path):
    try:
        ...
        # Gestion d'erreur plus robuste pour les valeurs manquantes
...
def build_optimized_lstm_model(input_shape, output_shape):
    ...
    x = Dropout(0.5)(x)  # Augmentation du taux de dropout
    x = LSTM(256, return_sequences=True)(x)
    x = BatchNormalization()(x)
    x = Dropout(0.5)(x)  # Augmentation du taux de dropout
    x = LSTM(128, return_sequences=True)(x)  # Ajout d'une quatrième couche LSTM
    x = BatchNormalization()(x)
    x = Dropout(0.5)(x)  # Augmentation du taux de dropout
...
def train_model_for_pair(json_file, output_dir='models', sequence_length=10, 
    ...
    early_stop = EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=True)  # Augmentation de la patience
    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=5, min_lr=1e-6)  # Réduction du facteur
    model_checkpoint = ModelCheckpoint(os.path.join(pair_dir, 'best_model.keras'), monitor='val_loss', save_best_only=True, mode='min') # Correction de l'extension
    history = model.fit(X_train_seq, y_train_seq, validation_data=(X_test_seq, y_test_seq), epochs=epochs, batch_size=batch_size, callbacks=[early_stop, reduce_lr, model_checkpoint], verbose=1)
    ...
    for tolerance in [0.001, 0.0005, 0.0001]:
        ...
        for i in range(len(y_true)):
            for j in range(len(y_true[i])):
                if abs(y_pred[i][j] - y_true[i][j]) / (abs(y_true[i][j]) + 1e-9) <= tolerance: # Ajout de 1e-9 pour éviter la division par zéro
...
def main():
    ...
    sequence_lengths = [10, 15, 20, 30, 50]  # Ajout de séquences plus longues
    test_ratio = 0.2
    batch_sizes = [8, 16, 32, 64, 128]  # Ajout de batch sizes supplémentaires
    epochs = 100
    ...
    for json_file in json_files:
        for seq_len in sequence_lengths:
            for batch_size in batch_sizes:
                ...
                train_model_for_pair(json_file, output_dir=models_folder, sequence_length=seq_len, test_ratio=test_ratio, batch_size=batch_size, epochs=epochs)
